...
def evaluation(minDistList, traj_, X, Y):
    ...
    timeBaseline = 250
    dt = 0.05
    Cmax_cl = 9.0           # constrain of maximun distance for completing the loop
    ...
    if comGrad < fs:
        ...
        print('Your time is ',
              timeCurrent)
        if(timeCurrent <= bonus_t):
            fbonus=10.0
            print("Bonus score: {}".format(fbonus))

    grade=avgGrad+maxGrad+comGrad+beatBaselineScore+fbonus
